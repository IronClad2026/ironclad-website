...
import OperationsNavigationBoundary from "./OperationsNavigationBoundary";
...
export default function AdminOperationsDashboard({ metrics, websiteTraffic }: { metrics: AdminOperationsMetrics; websiteTraffic: WebsiteTrafficAnalytics; }) {
  return (
    <main lang="en" className={styles.workspace + " relative z-10 min-h-screen min-w-0 bg-black px-4 pb-16 pt-28 text-white sm:px-6 sm:pt-32 lg:px-8"}>
      <OperationsNavigationBoundary>
        <a href="#operations-overview" className="sr-only focus:not-sr-only">Skip to current operations</a>
        <OperationsLiveOverview metrics={metrics} navigation={<details className="group rounded-xl border border-white/10 bg-zinc-950">
          <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between px-4 text-sm font-semibold">Jump to section <ChevronDown aria-hidden="true" className="h-4 w-4 group-open:rotate-180" /></summary>
