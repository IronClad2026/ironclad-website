...
export async function acknowledgeBadgeReveal(
  ...
  if (!userId) {
    return actionError(
      "sign-in-required",
      "Sign in before acknowledging a badge reveal."
    );
  }

  if (typeof awardId !== "string" || !UUID_PATTERN.test(awardId)) {
    return actionError("invalid-award", "This badge award is not valid.");
  }
  ...
  if (playerError) {
    ...
    return actionError(
      "lookup-failed",
      "Your badge reveal could not be verified. Please retry."
    );
  }
  ...
  if (!player || typeof player.id !== "string") {
    return actionError(
      "profile-required",
      "Complete your player profile before acknowledging badge reveals."
    );
  }
  ...
  if (awardError) {
    ...
    return actionError(
      "lookup-failed",
      "Your badge award could not be verified. Please retry."
    );
  }
  ...
  if (
    ...
    return actionError(
      "award-not-owned",
      "This badge award is not available to your player profile."
    );
  }
  ...
  if (existingRevealError) {
    ...
    return actionError(
      "lookup-failed",
      "Your badge reveal state could not be verified. Please retry."
    );
  }
  ...
  // A concurrent acknowledgement is equivalent to success.
  if (insertError && getErrorCode(insertError) !== "23505") {
    logAcknowledgeFailure("insert-reveal", insertError);
    return actionError(
      "acknowledge-failed",
      "Your badge reveal was not saved. Please retry."
    );
  }

  return acknowledgeSuccess(insertError ? "already-acknowledged" : "acknowledged");
}
