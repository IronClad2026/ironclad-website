...
Real hosted synthetic-player login passed on SHA `3dd084664ba074fd2871e3dd790ab6cc5d38010d` at `https://ironclad-website-4cd11z6at-ironclad-tournaments.vercel.app`. The temporary exact-deployment access link established a memory-only Vercel cookie while protection remained enabled. Clerk required its Development test email-code step after the actual password; the documented reserved test code completed a genuine session, whose identity matched the verified backend fixture. This single-case diagnostic is not complete release evidence. No account was created, no identity was promoted and no competition result was changed.
