...
Current environment-list verification was unavailable through the connected Vercel tools: get-project failed with an argument-schema mismatch and get-deployment omitted environment bindings. Cached CLI 59.25.0 is available, but its normal auth file contains no token and no Vercel token is configured in the process. A read-only CLI request unexpectedly offered device login; that flow was immediately cancelled. No Vercel setting was mutated by this audit.

Before a candidate push that can auto-deploy, independently establish the default Preview environment binding or configure explicit branch-only Preview overrides using the verified Staging values. Do not assume an override scoped to `staging` applies to `codex/p03-production-ready`. Set only Preview targets; never change Production entries. Verify both build-time public values and runtime server credentials. Keep email disabled for the candidate unless a separately isolated test delivery configuration is established, and do not inherit Production push, email, webhook, or Steam secrets. A command-line deployment can set explicit build/runtime overrides once CLI auth is available; keep secret values out of command logs and persisted artifacts.
