...
as $$
  select coalesce(
    (
      select tournament.status in ('in_progress', 'completed', 'closed')
        or not public.is_tournament_bracket_regeneration_safe(
...
revoke all on function public.is_tournament_bracket_roster_locked(uuid)
  from public;
grant execute
  on function public.is_tournament_bracket_roster_locked(uuid)
  to service_role;
