...
as $$
begin
  if new.event_key is not null or new.registration_id is null then
    return new;
  end if;

  if new.type = 'registration.waitlist_offer' then
    new.event_key := pg_catalog.format(
      'registration:%s:waitlist-offer',
      new.registration_id
    );
  elsif new.type = 'registration.waitlist_closed' then
    new.event_key := pg_catalog.format(
      'registration:%s:waitlist-closed',
      new.registration_id
    );
  end if;
...
alter function public.assign_canonical_notification_event_key()
  owner to postgres;
revoke all on function public.assign_canonical_notification_event_key()
  from public, anon, authenticated, service_role;

drop trigger if exists notifications_assign_canonical_event_key
  on public.notifications;
create trigger notifications_assign_canonical_event_key
before insert on public.notifications
for each row
execute function public.assign_canonical_notification_event_key();

create unique index if not exists
  notifications_admin_push_event_key_unique_idx
...
comment on index public.notifications_admin_push_event_key_unique_idx is
  'Deduplicates launch Push-important role-targeted Admin events by canonical event identity.';
...
begin
  ...
  select
    ...
    opponent.id,
    pg_catalog.coalesce(
      pg_catalog.nullif(pg_catalog.btrim(submitter.player_name), ''),
      'A player'
    ),
    tournament.title,
    match.match_number,
  ...
  if tg_op = 'INSERT' then
    ...
    set read_at = pg_catalog.coalesce(
...
drop trigger if exists
  match_result_report_groups_create_confirmation_notification
on public.match_result_report_groups;
create trigger match_result_report_groups_create_confirmation_notification
after insert on public.match_result_report_groups
for each row
execute function public.sync_match_confirmation_required_notification();

drop trigger if exists
  match_result_report_groups_resolve_confirmation_notification
on public.match_result_report_groups;
create trigger match_result_report_groups_resolve_confirmation_notification
after update of status, finalized_at on public.match_result_report_groups
for each row
execute function public.sync_match_confirmation_required_notification();
