alter table public.registrations
  add column if not exists elo_verified_elo integer,
  add column if not exists elo_difference integer,
  add column if not exists elo_highest_faction text,
  add column if not exists elo_checked_mode text,
  add column if not exists elo_checked_at timestamptz,
  add column if not exists elo_verification_source text,
  add column if not exists elo_verification_error text,
  add column if not exists elo_verification_payload jsonb,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'registrations_elo_verified_elo_check'
    ...
    alter table public.registrations
      ...
      check (
        ...
        or (elo_verified_elo >= 0 and elo_verified_elo <= 5000)
      );
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'registrations_elo_difference_check'
      and conrelid = 'public.registrations'::regclass
  ) then
    alter table public.registrations
      add constraint registrations_elo_difference_check
      check (elo_difference is null or elo_difference >= 0);
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'registrations_elo_highest_faction_check'
      and conrelid = 'public.registrations'::regclass
  ) then
    alter table public.registrations
      ...
      check (
        ...
        or elo_highest_faction in ('us', 'british', 'wehrmacht', 'dak')
      );
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'registrations_elo_checked_mode_check'
      and conrelid = 'public.registrations'::regclass
  ) then
    alter table public.registrations
      add constraint registrations_elo_checked_mode_check
      check (
        elo_checked_mode is null
        or elo_checked_mode in ('1v1', '2v2', '3v3', '4v4')
      );
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'registrations_elo_verification_source_check'
      and conrelid = 'public.registrations'::regclass
  ) then
    alter table public.registrations
      ...
      check (
        ...
        or elo_verification_source = 'coh3stats'
      );
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'registrations_elo_verification_payload_object_check'
      and conrelid = 'public.registrations'::regclass
  ) then
    alter table public.registrations
      add constraint registrations_elo_verification_payload_object_check
      check (
        elo_verification_payload is null
        or jsonb_typeof(elo_verification_payload) = 'object'
      );
  end if;
...
create or replace function public.is_elo_verification_enabled()
returns boolean
language sql
stable
security definer
...
as $$
  select coalesce(
    (
      ...
      where setting.key = 'elo_verification'
    ),
    false
  );
$$;

revoke all on function public.is_elo_verification_enabled()
...
comment on column public.registrations.elo_verified_elo is
  'Highest current COH3 Stats ELO found across factions for the checked mode.';
comment on column public.registrations.elo_difference is
  'Absolute difference between submitted_elo and elo_verified_elo.';
comment on column public.registrations.elo_highest_faction is
  'Faction that produced elo_verified_elo: us, british, wehrmacht, or dak.';
comment on column public.registrations.elo_checked_mode is
  'Tournament mode checked against COH3 Stats: 1v1, 2v2, 3v3, or 4v4.';
comment on column public.registrations.elo_checked_at is
  'Timestamp when the registration-time ELO verification check completed.';
comment on column public.registrations.elo_verification_source is
  'External ELO verification source, currently coh3stats.';
comment on column public.registrations.elo_verification_error is
  'Friendly failure reason when ELO verification could not complete.';
comment on column public.registrations.elo_verification_payload is
  'Structured registration-time COH3 Stats verification details for admin review.';
