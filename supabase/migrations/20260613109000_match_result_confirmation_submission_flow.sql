...
create table if not exists public.player_report_group_notification_dismissals (
  clerk_user_id text not null,
  report_group_id uuid not null
    references public.match_result_report_groups(id) on delete cascade,
  ...
  primary key (clerk_user_id, report_group_id, dismissed_status)
);

create index if not exists
  player_report_group_notification_dismissals_user_idx
  on public.player_report_group_notification_dismissals(
    clerk_user_id,
    dismissed_at desc
  );

alter table public.player_report_group_notification_dismissals
  enable row level security;
revoke all on table public.player_report_group_notification_dismissals
  from public;
revoke all on table public.player_report_group_notification_dismissals
  from anon, authenticated;
grant all on table public.player_report_group_notification_dismissals
  to service_role;
...
begin
  ...
  if p_replay_storage_path is null
    or btrim(p_replay_storage_path) = '' then
