...
describe("badge award reads", () => {
  it("maps real award rows into the dashboard badge contract", async () => {
    ...
    const badgeData = await buildDashboardBadgeDataFromAwards(
      fixture.client,
      PLAYER_ID
    );
    ...
    expect(badgeData.collection.earnedCount).toBe(1);
    expect(
  ...
  it("falls back to locked badges on read errors instead of inventing awards", async () => {
