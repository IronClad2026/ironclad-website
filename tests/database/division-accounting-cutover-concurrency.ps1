...
create function public.pr7_finalize_authenticated_fixture_match()
returns void
language sql
security definer
...
as `$`$
  ...
  set
    ...
    winner_registration_id =
      'f7300000-0000-4000-0005-000000000001'::uuid,
    status = 'completed'
  where id = 'f7600000-0000-4000-8000-000000000005'::uuid;
`$`$;
revoke all on function public.pr7_finalize_authenticated_fixture_match()
  from public, anon, authenticated, service_role;
grant execute on function public.pr7_finalize_authenticated_fixture_match()
  to authenticated;
"@
...
$authenticatedTrigger = Invoke-Psql -Sql @"
set session authorization authenticated;
begin;
select public.pr7_finalize_authenticated_fixture_match();
commit;
"@ -ApplicationName "ironclad-pr7-authenticated-trigger"
Assert-PsqlSuccess -Result $authenticatedTrigger -Label "authenticated result trigger"
...
$cleanup = Invoke-Psql -Sql @"
drop function public.pr7_finalize_authenticated_fixture_match();
"@
Assert-PsqlSuccess -Result $cleanup -Label "authenticated trigger fixture cleanup"
...
[pscustomobject]@{
  ...
  authenticatedTrigger = "completed through the existing guarded helper chain"
